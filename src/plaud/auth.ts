...
export class PlaudAuth {
  ...
  async getToken(): Promise<string> {
    ...
    const canRefresh = Boolean(creds?.email && creds?.password);
    ...
    if (cached) {
      // Token-only mode (browser-login): use until it 401s.
      return cached.accessToken;
    }

    throw new Error('No Plaud token. Run `npm run plaud:login` or `npm run plaud:browser-login` first.');
  }

  async login(): Promise<string> {
    const creds = this.config.getCredentials();
    if (!creds?.email || !creds?.password) {
      throw new Error('No credentials configured. Run `npm run plaud:login` first.');
    }
