...
# AI-Driven Development: Aligning on Tooling, Metrics, and Governance

![PLAUD NOTE](permanent/d71eb427242b4f82a4065349af9d8252/mem_cBpPZkdsFH/summary_poster/card_20260609135119-v2@c354a55084bd8a0a9f6d32_20260609135354_edab148c.png)


This multi-party working session examined how AI is impacting software delivery speed and code quality, how to quantify and manage technical debt using SIG metrics, and how to avoid fragmented practices by converging on a unified, adaptable development harness and governance model. The group agreed to form a core architecture team, define standardized inputs (PRDs/tickets), embed quality gates in pipelines, and establish a cadence for shared learning and tangible outputs.
------------
## Performance Metrics and AI's Impact on Productivity

The participants rely on Software Improvement Group (SIG) to quantify productivity and quality, with measures expressed in people-years, people-months, and people-days. Despite historically being “1.5 below” expected productivity, the team highlighted a concrete case: between May 1 and May 19, Jesper delivered “five people-months” of effort while holding a separate day job, illustrating AI’s potential to amplify throughput. Maintainability scores for the ticket system (front-end and back-end) fluctuate yet remain stably above 4.7. The group uses “volume effort” to estimate output in people-months and observed a sharp, initial drop in maintainability after AI-driven changes, prompting targeted remediation.

Quality safeguarding is being operationalized via an agent-enabled PR review workflow. Jesper added a prompt step that invokes an SQL “skill” to inspect metrics before raising a PR—akin to SonarCloud linting—improving maintainability without sacrificing pace. This behavior is now part of the PR review agent, reinforcing continuous improvement during development. The team clarified that metrics are not based on lines of code; instead they factor code complexity, expected auto-generation, and AI-assisted autocomplete contributions. While the newly introduced “AI code governance” feature is still in beta and estimates 30–40% “hands-off” contributions in some repos (which the group considers inaccurate and over-instructed), the consensus is to lean on industry-validated tools (SIG, SonarCloud) and meaningful metrics like churn that reveal whether targeted fixes truly stick.

------------
## Technical Debt and Strategic Refactoring

SIG reports quantify code volume and technical debt in person-years to frame strategic decisions. Current code volume was cited as “136 person years” (the time to reproduce the codebase). Historical data show a decline from “300–229” down to “136,” and an overall doubling of speed across the business. Prior figures included “95 man years” of volume and “41 man years” of technical debt across two combined contexts (also referenced as “one third” and “41.5 years” by different speakers). A SIG report about a year ago warned of increasing technical debt; in response, the team executed a focused sprint to address flagged issues, which ultimately resulted in “basically nothing really important” needing change—nonetheless reinforcing SIG’s utility as a trigger and oversight body. The group appreciates SIG’s “churn” metric to validate whether refactoring efforts actually resolve problems rather than oscillate.

------------
## Current State of AI Tooling and Development Workflow

Workflows currently mix custom agents and tooling with a strong separation of concerns. A Product Owner (PO) or product definition agent captures business requirements (functional and non-functional) without reading code. A refinement agent then adds technical specifications—API selections, contracts, integrations—and communicates with a development agent that is code-aware. The pipeline should transform refined definitions into actionable Jira work items and artifacts, enabling development agents to produce implementations quickly. A final review agent checks adherence to product definitions, verifies tests, and confirms outputs match the specification. The aspiration is “no coding task” after acceptance: once requirements are agreed and “continue” is signaled, agents carry out implementation and validation in minutes.

In practice, the PO agent today does not inspect code and requires repeated domain inputs due to memory limits, but produces testable scenarios. The development agent then evaluates and aligns code accordingly. Product definitions are intended to evolve in tandem with code, but remain separate from code-level inspection; cross-referencing will be across product definitions for related domains rather than codebases. The team stressed pre-coding refinement cycles, especially for external APIs and dependencies, to avoid late-found coupling issues. There is strong interest in common guardrails, policies, and structures (not necessarily reusing agents “one-to-one” due to differing tech stacks), and in reducing bespoke initiatives where each developer builds unique tools for their team.

------------
## Establishing a Unified Governance and Development Harness

A central concern was avoiding divergence—“two different ways of working, two different harnesses.” The proposed solution is a single, jointly produced governance model and a common harness shared across companies (e.g., Unique, Zeg), with pluggable components for tech stack specialization. The harness should start with baseline capabilities that apply to everyone and grow with domain-specific modules (e.g., CRM), enabling adaptable integration while preserving consistent principles. This includes unified approaches to gateways, memory management, and governance, embedded directly in day-to-day execution rather than only documented on paper.

The group endorsed building a combined team to design, implement, and validate the harness—working across organizations simultaneously rather than aligning principles and building separately. The end-state vision: a harness that can plug in PO/refinement/development agents and shared services, adapt to role and product context (“I’m a PO working for X”), and enable discoverability and secure interconnection (brainstormed ideas like profile-installed instructions or certificates). Practical alignment steps include polishing existing harnesses (e.g., Windows-centric ones, ensuring Mac compatibility) and agreeing on standardized inputs (PRDs/tickets) so AI agents receive consistent, consumable specifications.

------------
## Action Plan for Cross-Company Collaboration and Knowledge Sharing

The team agreed to constitute a small core with a mandate to architect a generic, adaptable framework (participants referenced Andrew, Wonen, and Emilio). The approach is layered: the core group will brainstorm the reference architecture and quickly pull in implementation experts to test feasibility against current technical practices. Once validated, outputs will feed into the international AI community through monthly knowledge-sharing sessions with demos, complemented by more frequent working syncs during intensive design phases.

Immediate tangible needs were identified: a customer-facing harness starting in quarter three, monitoring to observe trends by quarter’s end, and a standardized PO-side process for PRDs/tickets that produce AI-consumable outputs. The team will prepare a concrete roadmap derived from transcripts and prior notes, avoid vendor lock-in while selecting gateway/observability solutions (e.g., considering MLflow, LightLLM), and ensure observability not only for internal development but also client-side usage (e.g., chat on websites). To sustain momentum and transparency, the group will maintain a visible update channel (international community), while using smaller, focused chats for detailed coordination.

------------
## Action Items

- Owner groupings follow named speakers in the transcript where task ownership was implied or stated. All items include deadlines where explicit; otherwise [TBD] is applied per rule.

**@Kobbe van Daatselaar**
- [ ] Write down the concrete actions discussed and share them with the group - [TBD]
- [ ] Send an update to the international AI community summarizing the last two days, outcomes, next steps, and how they can be involved - [TBD]
- [ ] Use the existing monthly international knowledge-sharing session to ensure people working on AI give demos and updates - [TBD]
- [ ] Have a one-on-one with Jonathan from Activinov (Lyon) to discuss his involvement - [TBD]
- [ ] Discuss with Jonathan which group/track to place Activinov within this initiative - [TBD]
- [ ] Coordinate initial alignment to avoid “two different harnesses” by promoting combined governance and a single way of working - [TBD]

**@Speaker 5 (Enterprise security architect / core organizer)**
- [ ] Set up a regular sync cadence among the three core members to track progress - [TBD]
- [ ] Brainstorm the high-level reference architecture and quickly pull in experts to validate feasibility - [TBD]
- [ ] Establish smaller, focused chats (e.g., Teams) for detailed coordination and connect with Speaker 7 to discuss product definition inputs - [TBD]
- [ ] Prepare and iterate a concrete reference architecture, then share outputs with the international community on a monthly cycle - [TBD]
- [ ] Ensure the harness design supports pluggable specializations across different tech stacks without vendor lock-in - [TBD]
- [ ] Figure out where to fit Speaker 7 (PO agent developer) within regular technical platform activities - [TBD]

**@Emilio Oldenziel**
- [ ] Participate in the core trio (with Andrew and Wonen) to architect the generic framework and assemble the roadmap from recent transcripts/notes - [TBD]
- [ ] Define the AI gateway requirements and evaluate vendors (e.g., LightLLM, MLflow), minimizing lock-in risk - [TBD]
- [ ] Plan client-side observability (e.g., chat usage on websites) and integrate it with platform observability - [TBD]

**@Speaker 7 (PO agent developer)**
- [ ] Deliver a customer-facing harness starting in quarter three - [Quarter 3]
- [ ] Implement monitoring to reveal usage and quality trends by the end of the quarter - [End of Quarter]
- [ ] Define and adopt a standardized way of creating AI-consumable PRDs/tickets for PO workflows - [TBD]
- [ ] Coordinate with the core group to align top-down governance and adapt existing PO agent processes accordingly - [TBD]

**@Speaker 2 (Jesper/Ole context; development agent contributor)**
- [ ] Maintain the PR review agent’s prompt step that invokes SQL skills to check metrics and fix maintainability issues pre-PR, iterating as needed - [TBD]

**@Raimond Stokkel**
- [ ] Align on guardrails, policies, and structural elements observed in the other team’s meetings; update current agents or harness accordingly - [TBD]
- [ ] Participate in combined refinement sessions (with PO and agents) to accelerate specification quality and catch issues upfront - [TBD]

**@General Team / Cross-Company**
- [ ] Promote the current “port harness” to a polished, platform-agnostic starting point (Windows and Mac) for every team; share all tooling; create a CRM-ready variant - [TBD]
- [ ] Agree on a unified governance model and a single adaptable harness with pluggable components for different stacks/domains - [TBD]
- [ ] Integrate SIG/SonarCloud metrics and agentic PR checks to enforce maintainability and validate churn improvements - [TBD]
- [ ] Use the international AI community channel for visible progress updates; keep deep-dive discussions in smaller working chats - [TBD]
