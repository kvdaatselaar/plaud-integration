...
## Meeting Information
> Date: 2026-06-08 19:00:02
> Location: [Insert Location]
> Participants: Emilio Oldenziel, Raimond Stokkel, Kobbe van Daatselaar, Ron Koppelaar, Speaker 2, Speaker 4, Speaker 5, Speaker 8
## Meeting Notes
- Topic Title: MLflow demo and local tracing capabilities
  - Description:
    - MLflow demo captured local VS Code traces and LLM tool-loop calls, running locally via Python/Flask and installable with pip and “mlflow demo.”
    - MLflow manages datasets, evaluation runs (“judges”), prompts, and includes a bare-bones gateway; used for quick prototyping with local Docker and maintained by Databricks.
    - Clarified that MLflow runs locally and is separate from Airflow.
  - Conclusion:
    - MLflow is validated as a practical, local-first platform for tracing, evaluation, and prototyping.
- Topic Title: Gateways, routing, security, and PII handling
  - Description:
    - Multiple frameworks now include gateways (LangSmith, Langfuse, LangChain Lab) with security features and PII filtering.
    - OpenAI’s privacy filter can mask PII to enable global routing; EU-only routing requires configuration.
    - Cost tracing per tool call available in Langfuse; similar capabilities exist across the ecosystem.
  - Conclusion:
    - Gateways with PII masking and routing controls are essential; selection depends on required security features.
- Topic Title: Compliance and organizational control of traces
  - Description:
    - Concerns about storing traces in personal GitHub or VS Code history and implications upon job changes.
    - Recommendation for an organizational-level control plane to log and restrict access, provide statistics, and prevent shadow IT.
    - Emphasis on enabling engineers with easy official tooling to avoid noncompliant practices.
  - Conclusion:
    - Establish centralized, compliant trace logging and access controls at the organizational level.
- Topic Title: Planning deliverables and reference architecture
  - Description:
    - Goal to produce a tangible reference architecture for AI in the SDLC by combining existing Markdown files and using an agent to suggest architecture.
    - Raimond will generate an initial MD file with current and projected architecture covering SDLC, governance, endpoint security boundaries, and guardrails.
    - Agreement to produce an AI-assisted suggested reference architecture for discussion.
  - Conclusion:
    - Team will prepare MD inputs and generate an initial AI-assisted reference architecture for review.
- Topic Title: Harness framework and sub-agents approach
  - Description:
    - Discussion on a harness framework controlling context and model selection, minimizing context to only necessary information.
    - Sub-agents subscribe to structured harness outputs (product definition, keymaker, refinement, survey agents).
    - Emphasis on documentation, indexing, and stakeholder understanding of harness behavior.
  - Conclusion:
    - Adopt a harness-first approach with clear principles and agent subscriptions to structured outputs.
- Topic Title: Shared collaboration spaces and repositories
  - Description:
    - Need for shared spaces: Miro, SharePoint, wiki, and code repo; Kobbe has an international community team site and enterprise GitLab setup.
    - GitHub organizational policies may complicate external access; guests may be needed for Zeek group.
    - An “SDLC agents” repo exists; parallel discussion on creating a new GitHub repo and inviting participants as guests, with a wiki hosted in the repo or via Teams if licenses limit access.
  - Conclusion:
    - Use enterprise GitLab and existing collaboration tools while coordinating GitHub access and policies; proceed with GitHub repo creation and wiki setup where appropriate.
- Topic Title: Build Pipelines, Agents, and Infrastructure
  - Description:
    - Current tooling spans GitHub, Azure DevOps, Jenkins (in Docker), and build agents; AL Go works for single apps but scaling is resource-heavy.
    - Build machines: two with 96 GB each; ~8 build agents running; estimated cost around three hundred dollars a month; need more capacity for Jenkins agents; bandwidth and isolation considerations noted.
    - Pipeline structure includes build and release pipelines across multiple repos to assign responsibilities.
  - Conclusion:
    - Infrastructure is functional but resource-constrained; optimize agents and consider cost and scaling options.
- Topic Title: Code Structure and Agent Coverage
  - Description:
    - Aim to avoid duplicating agents; point to specific folders for key agents (product refinement, backend/frontend developer agents).
    - Risks include missing agent components and documentation gaps, with some files empty or outdated; need to identify and group relevant files.
  - Conclusion:
    - Focus on a subset of key agents, set clear folder references, and prioritize documentation to avoid unnecessary duplication.
- Topic Title: ERP/CRM Copies and Bugfix Builds
  - Description:
    - Mention of ERP/CRM copies, bugfix builds, and test copies for Asia; need joint review for additional checking and marking rows; clarified EP/CREA availability and distinctions.
  - Conclusion:
    - Further validation required on ERP/CRM copies and bugfix builds.
- Topic Title: Governance Workflow and Commit Rules
  - Description:
    - Proposed workflow: new → approved → committed by team → in progress → resolved → done (PO confirms).
    - Pushing to master is acceptable if it doesn’t affect production; PO approvals required.
  - Conclusion:
    - Adopt structured workflow with PO approval at completion.
- Topic Title: Memory Tools and Context Management (MemPalace)
  - Description:
    - Citrus uses a memory tool on top of MemPalace with an added layer for hive mind context; via MCP, long-term context can be retrieved for tickets/specs/plans across machines.
    - Mixed experiences: concept useful; prior implementation quality issues recognized.
  - Conclusion:
    - Consider leveraging the concept while improving implementation quality and context limits.
- Topic Title: SSH Key Management for Agents
  - Description:
    - Decision pending on using an SSH keychain vs. agent-managed SSH; concerns about sharing SSH keys and HTTPS impersonation.
  - Conclusion:
    - Define a secure, standardized SSH approach.
- Topic Title: DevOps Team Structure
  - Description:
    - Clarified a single DevOps team with overlaps in ERP operations; need to define responsibilities across repos and teams.
  - Conclusion:
    - Recognize single DevOps team and align responsibilities.
- Topic Title: LLM Tooling, Costs, and Local Hosting
  - Description:
    - Plan to discuss LLM tooling costs; budgets set before deciding to scale AI; desire to run local LLMs on a farm (not laptops), seeking pre-token strategies and experienced support.
    - Experiments with AI Foundry and DeepSeek; DeepSeek works but has limited context window.
  - Conclusion:
    - Further exploration required on cost-effective models, local hosting, and optimization strategies.
- Topic Title: Presentation and demonstration plans
  - Description:
    - Planned demo of an agent creating PowerPoints using shared “diagram/presentation” skills; visual quality may be weak, focus on conveying the message.
    - Target to have materials prepared by 2026-06-09 morning for discussion.
  - Conclusion:
    - Proceed with demo emphasizing functionality over aesthetics.
- Topic Title: Meeting Logistics and Next-Day Agenda
  - Description:
    - Wiki created and link shared in team chat.
    - Next day plan (2026-06-09): start with agent governance framework; discuss LLM tooling costs; “way of working” session with metrics development manager; wrap up and define next steps.
  - Conclusion:
    - Agenda set for 2026-06-09 with governance, costs, and process sessions.
## Next Arrangements
- [ ] Combine existing MD files and generate an AI-assisted reference architecture suggestion.
- [ ] Raimond to produce an MD file with current architecture and projected design.
- [ ] Set up shared Miro team space and link it to SharePoint for collaborative work.
- [ ] Kobbe to add participants to the international community team site and configure a shared folder.
- [ ] Create or confirm access to the enterprise GitLab repo for SDLC agents and related materials; proceed with GitHub repo creation, wiki setup, and guest invites as needed.
- [ ] Define harness principles (minimal context, model control) and list target sub-agents (product definition, keymaker, refinement, survey).
- [ ] Prepare demo materials and agent-driven PowerPoint for review on 2026-06-09.
- [ ] Clarify gateway selection and PII filtering approach for compliant global routing.
- [ ] Define folder references for key agents and clean up outdated/empty files; document agent components.
- [ ] Validate ERP/CRM bugfix builds and perform required checks.
- [ ] Align and document the governance workflow (statuses and PO approval) in the wiki.
- [ ] Decide on SSH approach (SSH keychain vs. agent-managed SSH) for repository access.
- [ ] Assess Jenkins and build agent resource needs, bandwidth, and isolation; consider scaling options.
- [ ] Prepare for the 2026-06-09 sessions on agent governance, LLM tooling costs, and way of working.
## AI Suggestions
> 1. Define the exact reference architecture scope and components (SDLC coverage, governance, security boundaries) to avoid ambiguity.
> 2. Choose and document the gateway solution (MLflow gateway vs. LangSmith/Langfuse/LangChain Lab) with required security and PII features.
> 3. Establish a formal compliance policy for trace storage, retention, and offboarding to prevent data leakage from personal repositories.
> 4. Confirm the shared repository location (enterprise GitLab vs. GitHub) and finalize external access policies and guest account processes; ensure all invites are completed.
> 5. Prioritize documentation for key agents and clean up outdated or empty files to prevent delays.
> 6. Define a secure, standardized SSH key management approach to mitigate credential risks.
> 7. Develop a resource scaling plan for Jenkins/build agents with budget alignment and bandwidth/isolation considerations.
> 8. Create an LLM hosting strategy for a local model farm, including model selection, context window needs, cost controls, and pre-token optimization.
